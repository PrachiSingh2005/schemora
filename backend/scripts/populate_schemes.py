...
async def main():
    parser = argparse.ArgumentParser(description="Populate Schemora Knowledge Base using Groq AI")
    parser.add_argument("--api-key", type=str, help="Groq API Key", default=None)
    ...
    api_key = args.api_key or settings.GROQ_API_KEY or os.environ.get("GROQ_API_KEY")
    ...
    if not api_key:
        logger.warning("No GROQ_API_KEY provided. Skipping Groq dynamic generation. Set GROQ_API_KEY in .env or pass --api-key.")
        return

    logger.info(f"Using Groq API to generate schemes across categories: {args.categories}")
    ...
    for cat in args.categories:
        try:
            ...
            schemes = await generate_schemes_with_groq(api_key=api_key, category=cat, count=args.count_per_category)
            for s in schemes:
